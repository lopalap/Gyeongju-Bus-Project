...
테스트 항목,기대 결과,실제 결과
회원가입/로그인,사용자 ID/PW를 입력하여 성공 시 메인 지도 화면으로 이동한다.,정상 동작
정류장 검색 (초성),'ㄱㄷ' 입력 시 '경주동국대' 등 관련 정류장 목록이 정상 출력된다.,정상 동작
지도 마커 표시,검색된 정류장 위치에 Kakao Map 마커가 정확히 표시된다.,정상 동작
실시간 정보 조회,마커 클릭 시 해당 정류장의 버스 도착 예정 시간(API 데이터)이 팝업으로 뜬다.,정상 동작
즐겨찾기 등록/해제,정류장 팝업의 ★ 버튼 클릭 시 DB에 저장/삭제되며 색상이 변경된다.,정상 동작
즐겨찾기 목록 확인,홈 화면의 즐겨찾기 탭 클릭 시 저장된 정류장 목록만 즉시 조회된다.,정상 동작
도착 알림 설정,"특정 버스 노선에 대해 알림을 설정하면, 잔여 시간이 계산되기 시작한다.",정상 동작
3분 전 푸시 알림,버스 도착 예정 시간이 180초(3분) 이하가 되면 브라우저 알림이 발생한다.,정상 동작
데이터 실시간 갱신,설정된 주기(예: 10초)마다 API를 재호출하여 버스 위치와 시간이 업데이트된다.,정상 동작
